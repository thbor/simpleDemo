...
import {oneLevelLineData,getTypeShowData} from './newUtils'
...
const getTargetRateArr=(data:any,arr:any)=>{
	...
	let targetKeys = getTargetKeys(data.prodType)
	if(rate==="比率"){
...
const getTargetKeys=(prodType:any)=>{
	let allKeys = [{name:"良品",key:'good'},{name:"不良品",key:'bad'},{name:"報廢品",key:'scrap'}]
	let targetKeys = []
	for(let item of prodType){
		let index = allKeys.findIndex((aItem:any)=>aItem.name === item)
		if(index>-1){
			let value = allKeys[index].key
...
const getValueRateByRes=(res:any,generatedata:any,lastLevels:any,paramsData:any)=>{
	let itemValue = paramsData.itemList[0]
	//levelNum:需要過濾的層數
	for(let item1 of res){
		...
		for(let i=0;i<data.length;i++){
			...
			let currentKey = item2.levelName1+","+item2.levelName2+","+(item2.levelName3||"NG")
			//不重複的x軸的key
			...
			if(index>-1){
				...
				arr.push({key:currentKey,uniXKey:uniXKey,count:1,good:0,bad:0,scrap:0,goodRate:0,badRate:0,scrapRate:0,levelName1:item2.levelName1,levelName2:item2.levelName2,levelName3:item2.levelName3||"NG"})
				checkType(item2,arr[arr.length-1],itemValue)
			}
		}
		for(let aItem of arr){
			let count = getCurrentCountNG(aItem,generatedata)
			aItem.count = count
			aItem.goodRate = Number(((aItem.good/aItem.count)*100).toFixed(2))
			aItem.badRate = Number(((aItem.bad/aItem.count)*100).toFixed(2))
			aItem.scrapRate = Number(((aItem.scrap/aItem.count)*100).toFixed(2))
		}
		...
		for(let item2 of unikeys2){			
				...
				if(index>-1){
					...
					let ZeroItem={
						...
						goodRate: 0,
						key: item2+","+item.key,
						levelName1: item2.split(",")[0],
						levelName2: item2.split(",")[1],
						levelName3: item.key,
						scrap: 0,
						scrapRate: 0,
						uniXKey: item2
...
const getXAxis=(lastLevels:any,data:any,newres:any)=>{
	...
	// //2層的key
	let levelNum = lastLevels.level
	let levelName1s = arr.map((item:any)=>item.split(",")[0])
	let levelName2s = arr.map((item:any)=>item.split(",")[1])
	if(levelNum===3){
...
const getX=(data:any,levelName2s:any,number:number,offsetIndex?:number)=>{
	let nameIndex2 = itemTypeInfos.findIndex((item:any)=>item.itemType === data["conLevel"+number].itemType)
	let lastLevels:lastLevels = getLastLevels(data)
